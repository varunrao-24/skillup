    ...
    useEffect(() => {
        ...
        const searchStudents = async () => {
            ...
            try {
                ...
                const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/admin/students/search`, {
            ...
            <div className="p-2 border border-slate-300 rounded-md">
                <div className="flex flex-wrap gap-2 mb-2">
                    {selectedStudents.map(student => (
                        <div key={student._id} className="bg-blue-100 text-blue-800 text-sm font-medium px-2 py-1 rounded-full flex items-center gap-2">
                            {student.firstName} {student.lastName} ({student.username})
                            <button onClick={() => removeStudent(student._id)} className="text-blue-600 hover:text-blue-900"><X size={14} /></button>
                        </div>
                    ))}
                </div>
                            ...
                            {selectedResults.length > 0 && (
                                <div className="p-2 border-t border-gray-200 bg-gray-50">
                                    <button
                                        ...
                                        className="w-full bg-blue-600 text-white py-1.5 px-3 rounded-md hover:bg-blue-700 transition-colors text-sm font-medium"
                                    >
...
function AdminBatches() {
    ...
    const fetchBatches = useCallback(async () => {
        ...
        try {
            ...
            const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/admin/batches/all`, {
    ...
    useEffect(() => {
        document.title = "Admin | Manage Batches";
        fetchBatches();
    }, [fetchBatches]);
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            if (modalState.type === 'add') {
                await axios.post(`${import.meta.env.VITE_API_URL}/api/admin/batches/add`, payload, { headers });
                toast.success('Batch created successfully!');
            } else if (modalState.type === 'edit') {
                await axios.put(`${import.meta.env.VITE_API_URL}/api/admin/batches/update/${modalState.data._id}`, payload, { headers });
    ...
    const handleDelete = async () => {
        ...
        try {
            const token = Cookies.get('token');
            await axios.delete(`${import.meta.env.VITE_API_URL}/api/admin/batches/delete/${modalState.data._id}`, { headers: { Authorization: `Bearer ${token}` } });
                        ...
                        <tbody>
                            {isLoading ? (
                                ...
                                <tr key={batch._id} className="border-b border-slate-100 hover:bg-slate-50/70">
                                    ...
                                    <td className="p-4">
                                        <button onClick={() => openModal('edit', batch)} className="text-blue-600 p-2 rounded-full hover:bg-blue-100">
                                            <Edit size={18} />
                                        </button>
            ...
            <Modal isOpen={modalState.type === 'add' || modalState.type === 'edit'} onClose={closeModal} title={modalState.type === 'add' ? 'Create New Batch' : 'Edit Batch'}>
                <form onSubmit={handleFormSubmit} className="space-y-4 mt-4">
                    ...
                    <div className="flex justify-end gap-3 pt-4">
                        ...
                        <button type="submit" disabled={isSubmitting} className="px-4 py-2 bg-blue-600 text-white rounded-lg disabled:opacity-50">{isSubmitting ? 'Saving...' : 'Save Batch'}</button>
                    </div>
                </form>
            </Modal>
