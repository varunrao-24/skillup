...
const UserAvatar = ({ user, size = 'w-8 h-8' }) => (
    user?.photo ?
        ...
        :
        <div className={`${size} rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold`}>
        ...
        const searchCourses = async () => {
            if (searchTerm !== selectedItem?.label) {
                try {
                    ...
                    const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/admin/search/task-assignables`, {
                        headers: { Authorization: `Bearer ${token}` },
                        params: { type: 'course', q: debouncedSearch }
                    });
        ...
        <div className="relative">
            ...
            {showResults && results.length > 0 && (
                <ul className="absolute z-10 w-full bg-white border rounded-md mt-1 max-h-48 overflow-y-auto shadow-lg">
                    {results.map(item => (
                        <li key={item._id} onMouseDown={() => selectItem(item)} className="px-3 py-2 hover:bg-blue-50 cursor-pointer">{item.title} ({item.courseCode})</li>
                    ))}
                </ul>
            )}
        </div>
...
function AdminTasks() {
  ...
  const navigate = useNavigate();
    ...
    const fetchTasks = useCallback(async () => {
        ...
        try {
            ...
            const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/admin/tasks/all`, {
    ...
    const openDetailsModal = async (taskId) => {
        try {
            const token = Cookies.get('token');
            const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/admin/tasks/details/${taskId}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            ...
            if (modalState.type === 'add') {
                await axios.post(`${import.meta.env.VITE_API_URL}/api/admin/tasks/add`, payload, { headers });
                toast.success('Task created successfully!');
            } else if (modalState.type === 'edit') {
                await axios.put(`${import.meta.env.VITE_API_URL}/api/admin/tasks/update/${modalState.data._id}`, payload, { headers });
    ...
    const handleDelete = async () => {
        ...
        try {
            const token = Cookies.get('token');
            await axios.delete(`${import.meta.env.VITE_API_URL}/api/admin/tasks/delete/${modalState.data._id}`, { headers: { Authorization: `Bearer ${token}` } });
                            ...
                            <motion.div key={task._id} className="bg-white rounded-xl shadow-lg border overflow-hidden flex flex-col group">
                                <div className="relative">
                                    ...
                                    <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button onClick={() => openModal('edit', task)} className="bg-white/80 p-1.5 rounded-full text-blue-600 hover:bg-white"><Edit size={16}/></button>
                        ...
                        <ul className="space-y-2">
                            {taskDetails.task.attachments?.length > 0 ? taskDetails.task.attachments.map(file => (
                                <a href={file.url} target="_blank" rel="noopener noreferrer" key={file.fileName}
                                    ...
                                    <span className="text-sm text-blue-700 underline truncate">{file.fileName}</span>
                                </a>
  ...
  />
                    ...
                    <div>
                        ...
                        <input type="file" multiple onChange={handleAttachmentChange} className="w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" />
                        <div className="mt-2 space-y-1">
                            ...
                            {selectedAttachments.map(file => <div key={file.name} className="text-xs flex items-center justify-between bg-blue-100 p-1.5 rounded"><span>{file.name}</span><button type="button" onClick={() => removeNewAttachment(file.name)}><X size={14}/></button></div>)}
                        </div>
                    </div>
