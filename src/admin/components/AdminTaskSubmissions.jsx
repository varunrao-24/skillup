...
const SubmissionStatus = ({ submission }) => {
    ...
    if (submission.status === 'Graded') return <span className="flex items-center gap-1.5 text-xs font-medium text-blue-700"><CheckCircle size={14} /> Graded</span>;
...
function AdminTaskSubmissions() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/admin/tasks/${taskId}/grades`, {
    ...
    useEffect(() => {
        document.title = "Admin | Grade Task";
        fetchData();
    }, [fetchData]);
    ...
    const handleSaveChanges = async () => {
        ...
        try {
            const token = Cookies.get('token');
            await axios.post(`${import.meta.env.VITE_API_URL}/api/admin/tasks/${taskId}/grade`, 
    ...
    return (
        <>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                <button onClick={() => navigate('/admin/tasks')} className="flex items-center gap-2 text-slate-600 hover:text-slate-900 mb-6 font-medium">
            ...
            <tr key={grade._id} className="border-b border-slate-100 last:border-b-0">
                ...
                <td className="p-4 text-right">
                    <button onClick={() => setEditableRow(isEditing ? null : grade.student._id)} 
                        className={`p-2 rounded-full transition-colors ${isEditing ? 'bg-green-100 text-green-700' : 'text-blue-600 hover:bg-blue-100'}`}>
                    ...
                    <div className="flex justify-end mt-6">
                        <button onClick={handleSaveChanges} disabled={isSubmitting}
                            className="px-6 py-2.5 bg-blue-600 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 disabled:opacity-50 flex items-center gap-2">
